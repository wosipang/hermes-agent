...
# -- Connection lifecycle -----------------------------------------------
    ...
    async def connect(self) -> bool:
        ...
        # DNS 解析 → 收集 IP 到池子 → 从 IP 池获取可达 IP
        proxy_url = None
        try:
            from gateway.platforms._proxy_ip_pool import ProxyIPPool
        except ImportError:
            ProxyIPPool = None
        ...
        if ProxyIPPool:
            ...
            # DNS 解析收集
            try:
                addrs = socket.getaddrinfo("proxy3.bj.petrochina", None)
                abnormal = ("fe80:", "169.254.", "127.", "0.", "10.33.176.")
                valid = [a[4][0] for a in addrs if not any(a[4][0].startswith(p) for p in abnormal)]
                for ip in valid:
                    pool.add_ip(ip)
                logger.info("[%s] DNS collected proxy IPs: %s", self.name, valid)
            except Exception as e:
                logger.debug("[%s] DNS resolution skipped: %s", self.name, e)

            # 从池子找可达 IP
            reachable_ip = pool.get_reachable(port=proxy_port, timeout=3)
            if reachable_ip:
                proxy_url = "http://{}:{}".format(reachable_ip, proxy_port)
                logger.info("[%s] Proxy reachable from pool: %s", self.name, proxy_url)
            else:
                logger.info("[%s] No reachable proxy in pool, using direct connection", self.name)
        else:
            logger.info("[%s] ProxyIPPool not available, using direct connection", self.name)
        ...
        try:
            ...
            # Set proxy env vars for dingtalk-stream SDK internal connections
            # (requests.post in open_connection, websockets.connect in _run_stream)
            if proxy_url:
                os.environ["HTTPS_PROXY"] = proxy_url
                os.environ["WSS_PROXY"] = proxy_url
                logger.info(
                    "[%s] Proxy env vars set for DingTalk stream: %s",
                    self.name,
                    proxy_url,
                )
            else:
                # Clean up proxy env vars if previously set
                os.environ.pop("HTTPS_PROXY", None)
                os.environ.pop("WSS_PROXY", None)
                logger.info(
                    "[%s] Proxy unavailable, using direct connection for DingTalk stream",
                    self.name,
                )
...
# Cancel any in-flight background tasks (emoji reactions, etc.)
        ...
        # Clean up proxy env vars set for the DingTalk stream
        # so other services are unaffected
        for _var in ["HTTPS_PROXY", "WSS_PROXY"]:
            os.environ.pop(_var, None)
        logger.info("[%s] Proxy env vars cleaned up", self.name)
...
# Extract text content
        ...
        # For voice/audio messages without text, try DingTalk server-side ASR first
        if not text and msg_type == MessageType.VOICE:
            rec_text = self._extract_voice_recognition(message)
            if rec_text:
                text = rec_text
                logger.info("[%s] Voice ASR (server): %s", self.name, text[:80])
            else:
                # No ASR available — use placeholder so agent responds
                text = "[语音消息]"
                logger.info("[%s] Voice message (no ASR), using placeholder", self.name)
...
# Do NOT strip "@bot" from the text.  The mention is a routing
    ...
    @staticmethod
    def _extract_voice_recognition(message: "ChatbotMessage") -> str:
        """Extract DingTalk server-side ASR text from a voice (audio) message.

        DingTalk provides its own speech recognition in
        ``message.extensions[\"content\"][\"recognition\"]`` for voice messages.
        Using this avoids the download→convert→transcribe pipeline entirely.
        """
        ...
        if isinstance(extensions, dict):
            ...
            if isinstance(content, dict):
                rec_text = (
                    ...
                    or content.get("recognition_text")
                    or ""
                )
    ...
    def _extract_media(self, message: "ChatbotMessage"):
        ...
        if msg_type_str == "picture" and not media_urls:
            msg_type = MessageType.PHOTO
        elif msg_type_str == "audio" and not media_urls:
            # DingTalk sends pure voice messages as message_type="audio"
            # with the download code in extensions.content.downloadCode
            msg_type = MessageType.VOICE
    ...
    async def _resolve_media_codes(self, message: "ChatbotMessage") -> None:
        ...
        if not codes_to_resolve:
            # 3. Extensions.content.downloadCode (for audio/voice messages)
            extensions = getattr(message, "extensions", None)
            if isinstance(extensions, dict):
                content = extensions.get("content", {})
                if isinstance(content, dict):
